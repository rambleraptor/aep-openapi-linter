...
beforeAll(async () => {
  linter = await linterForAepRule('0134', 'aep-134-response-body');
  return linter;
});

test('aep-134-response-body should find errors', () => {
  const oasDoc = {
    openapi: '3.0.3',
    paths: {
      '/test1/{id}': {
  ...
  return linter.run(oasDoc).then((results) => {
    ...
    expect(results).toContainMatch({
      path: ['paths', '/test2/{id}', 'patch', 'responses', '200'],
      message: 'The response body is not an AEP resource.',
    });
    expect(results).toContainMatch({
      path: ['paths', '/test3/{id}', 'patch', 'responses', '200', 'content', 'application/json'],
      message: 'The response body is not an AEP resource.',
    });
    expect(results).toContainMatch({
      path: ['paths', '/test4/{id}', 'patch', 'responses', '200', 'content', 'application/json', 'schema'],
      message: 'The response body is not an AEP resource.',
    });
    expect(results).toContainMatch({
      path: ['paths', '/test5/{id}', 'patch', 'responses', '200', 'content', 'application/json', 'schema'],
      message: 'The response body is not an AEP resource.',
    });
  });
});

test('aep-134-response-body should find no errors', () => {
  const oasDoc = {
    openapi: '3.0.3',
    paths: {
      '/test1/{id}': {
